...
parser = argparse.ArgumentParser(description='Inpainting images')
...
if __name__ == '__main__':
    ...
    with torch.no_grad():
        ...
        for m, model in enumerate(args.models):
            ...
            if m==0:
                batch_idx = np.random.randint(50)
                iterator = iter(dataloader)
                for b in range(batch_idx):
                    batch = next(iterator)
                batch = [b[:num_images].to(pl_module.device) for b in batch]
            ...
            reconst = torch.sigmoid(pl_module.decoder(z)).reshape(num_images, samples, -1)
            
            reconst = reconst.mean(1)

            if args.dataset == 'mnist':
                reconst = torch.round(reconst)

            pl_module.train()
        
            images = x*observed_x
                
            original = original[:num_images].reshape(-1, channels, size, size)
            images = images[:num_images].reshape(-1, channels, size, size)
            reconst = reconst[:num_images].reshape(-1, channels, size, size)

            inpainted = images.clone().detach()
            mask = mask[:num_images].reshape_as(inpainted)
    ...
    if not os.path.isdir('{}/experiments/figs/inpainting/'.format(LOGDIR)):
        os.mkdir('{}/experiments/figs/inpainting/'.format(LOGDIR))

    str_title = f"{pl_module.__class__.__name__}_inpainting"
    torchvision.utils.save_image(grid, '{}/experiments/figs/inpainting/inpainting_{}_{}.png'.format(LOGDIR, args.dataset, batch_idx))
