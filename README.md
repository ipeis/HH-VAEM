...
```
This will automatically download the <code>boston</code> dataset, split in 10 train/test splits and train HH-VAEM on the training split <code>0</code>. Two folders will be created: <code>data/</code> for storing the datasets and <code>logs/</code> for model checkpoints and [TensorBoard](https://www.tensorflow.org/tensorboard) logs, respectively. The variable <code>LOGDIR</code> can be modified in <code>src/configs.py</code> to change the directory where these folders will be created (this might be useful for avoiding overloads in network file systems).
