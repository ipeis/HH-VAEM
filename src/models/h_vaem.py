...
class HVAEM(HVAE):
    """
    Implements a Hierarchical VAEM (H-VAEM) as described in https://arxiv.org/abs/2202.04599

    """
    def __init__(self, 
            dataset: str, dim_x: int, dim_y: int, arch:str='base', dim_h: int=256,
            likelihood_y: str='gaussian', variance: float=0.1, imbalanced_y: bool=False,
            categories_y: int=1, prediction_metric: str='rmse',
            batch_size: int=128, lr: float=1e-3, samples_MC: int=1, data_path: str='../data/', split_idx: int=0,
        ...
        super(HVAEM, self).__init__(dataset=dataset, dim_x=dim_x, dim_y=dim_y, 
        arch=arch, dim_h=dim_h, likelihood_y = likelihood_y, variance=variance, imbalanced_y = imbalanced_y,
        categories_y=categories_y, prediction_metric=prediction_metric, 
        batch_size=batch_size, lr=lr, samples_MC = samples_MC, 
        data_path=data_path, split_idx=split_idx, 
        latent_dims=latent_dims, sr_coef=sr_coef, balance_kl_steps=balance_kl_steps, 
        anneal_kl_steps=anneal_kl_steps, update_prior=update_prior
        )
