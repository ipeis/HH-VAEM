...
class MIWAE(BaseVAE):
    """
    Implements the MIWAE https://arxiv.org/pdf/1812.02633.pdf
    ...
    def __init__(self, 
        ...
        batch_size=128, lr=1e-3, samples_MC = 10, data_path='../data/', split_idx=0):
        """
        MIWAE initialization
        ...
        super(MIWAE, self).__init__(dataset=dataset, dim_x=dim_x, dim_y=dim_y, 
    ...
    def forward(self, batch: tuple, samples: int) -> tuple:
        ...
        zx = torch.cat([z,x_hat],dim=-1)
         ...
         # MIWAE ELBO 
        prior = self.prior(z, observed)
        logp_x = self.decoder.logp(xt, observed_x, z=z, theta=theta_x).sum(-1)
        logp_y = self.predictor.logp(yt, observed_y, z=zx).sum(-1)
        log_p = logp_x + logp_y + prior
        ...
        logp_x = logp_x[logp_x!=0].mean()
        logp_y = logp_y[logp_y!=0].mean()

        kl_mean = torch.zeros(len(kls)).type_as(logp_x)
        for l, kl in enumerate(kls):
            kl_mean[l]= kl[kl!=0].mean()

        return -elbo, logp_x, logp_y, kl_mean
